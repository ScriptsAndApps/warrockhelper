    ...
    public partial class Form1:Form {
        ...
        private void timer1_Tick(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                graphics.CopyFromScreen((screencenterx - 19) + centerimagex, (screencentery + detectionheight) + centerimgy, 0, 0, bs.Size);
            ...
            for (int ii = 0; ii < 31; ii++)
            {
                if (detected(ii))
                {
                    ...
                    if (debugg) bs.SetPixel(bx1 + ii, by1 + 1, Color.Green);
                    countDetectedPixels++;
                }
                if (detected2(ii))
                {
                    //draw detected pixels
                   if (debugg) bs.SetPixel(bx1 + ii, by1 + 2, Color.Green);
                ...
                if (countDetectedPixels + countDetectedPixels2 > 3 && countDetectedPixels + countDetectedPixels2 <= 25)
                {
                    ...
                    if (debugg) bs.SetPixel(bx1 + ii, by1, Color.White);
            }
            if (debugg)
            {
